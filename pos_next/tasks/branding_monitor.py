...
def monitor_branding_integrity():
	...
	try:
		...
		if not doc.encrypted_signature:
			try:
				if hasattr(doc, "generate_signature"):
					...
					frappe.db.commit()
					frappe.logger().info("BrainWise Branding - Missing signature auto-generated by monitor task")
				else:
					frappe.log_error(
						title="BrainWise Branding - Missing Signature",
						message="Branding configuration is missing encrypted signature and no generator is available."
					)
			except Exception as regen_err:
				frappe.log_error(
					title="BrainWise Branding - Failed to Auto-Generate Signature",
					message=f"Branding configuration missing encrypted signature and regeneration failed: {regen_err}"
				)
