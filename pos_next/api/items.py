...
def price_check_item(scan_code, pos_profile=None):
	try:
		scan_code = (scan_code or "").strip()
		if not scan_code:
			return {"found": False, "scan_code": scan_code}
		...
		if barcode_data:
			item_code = barcode_data.get("parent")
			barcode_uom = barcode_data.get("uom")
		else:
			item_code = frappe.db.get_value("Item", {"name": scan_code}, "name")
			barcode_uom = None
		...
		item = frappe.db.get_value(
			"Item",
			item_code,
			["name as item_code", "item_name", "image", "stock_uom", "disabled", "is_sales_item"],
			as_dict=True,
		)
		...
		uom_candidates = []
		if barcode_uom:
			uom_candidates.append(barcode_uom)
		if item.get("stock_uom") and item.get("stock_uom") not in uom_candidates:
			uom_candidates.append(item.get("stock_uom"))
		uom_candidates.append(None)
		...
		price_list_rate = flt(price_row.get("price_list_rate")) if price_row else 0
		price_currency = price_row.get("currency") if price_row else currency
		price_uom = price_row.get("uom") if price_row else (barcode_uom or item.get("stock_uom"))
		...
		frappe.log_error(frappe.get_traceback(), "Price Check Item Error")
		return {"found": False, "scan_code": scan_code}
