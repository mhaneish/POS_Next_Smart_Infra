...
def _set_payment_account(payment, account):
    if not payment or not account:
        return
    if isinstance(payment, dict):
        payment["account"] = account
        return
    if hasattr(payment, "set"):
    ...
    try:
        ...
        try:
            ...
            frappe.log_error(
                f"Failed assigning account on payment row: {assign_err}. Primary error: {e}",
...
def update_invoice(data):
    ...
    try:
            ...
            for payment in invoice_doc.payments:
                ...
                if mode_of_payment and not payment.get("account"):
                    try:
                        ...
                        if account_info:
                            _set_payment_account(payment, account_info.get("account"))
                    except Exception as e:
                        frappe.log_error(
        ...
        # Guard: allow return without invoice only when enabled in POS Settings
        if (data.get("is_return") or invoice_doc.get("is_return")) and not invoice_doc.get("return_against"):
            pos_profile_name = invoice_doc.get("pos_profile") or pos_profile
            if pos_profile_name:
                ...
                if not allow_without_invoice:
                    frappe.throw(_("Return without invoice is not enabled for this POS Profile"))
