...
def get_branding_config():
	...
	try:
		...
		if not doc.encrypted_signature and hasattr(doc, "generate_signature"):
			try:
				doc.generate_signature()
				updates = {"encrypted_signature": doc.encrypted_signature}
				if getattr(doc, "encryption_key", None):
					updates["encryption_key"] = doc.encryption_key
				frappe.db.set_value("BrainWise Branding", doc.name, updates, update_modified=False)
				frappe.db.commit()
			except Exception as e:
				frappe.log_error(
					f"Branding signature regeneration failed: {str(e)}",
					"BrainWise Branding API"
				)
				return get_default_config()
