...
const CONFIG = {
	...
	PING_TIMEOUT_MS: 1500,       // Fast detection for local/internal networks
	PING_RETRY_COUNT: 1,

	// Stability buffer - prevents flapping
	OFFLINE_THRESHOLD: 1,      // Immediate offline on hard failure
	ONLINE_THRESHOLD: 2,       // Consecutive successes before going online

	// Adaptive intervals
	INTERVAL_ONLINE_MS: 5000,       // Check every 5s instead of 30s for fast detection
	INTERVAL_OFFLINE_MS: 5000,      // 5s when offline
	INTERVAL_UNSTABLE_MS: 2000,     // 2s when connection is unstable
...
class NetworkMonitor {
	constructor() {
		...
		this._lastPingSuccess = 0 // Timestamp of last successful ping
	...
	async _performPing() {
		...
		for (let attempt = 1; attempt <= CONFIG.PING_RETRY_COUNT; attempt++) {
			try {
				...
				// Cache-busting ping URL with timestamp
				const pingUrl = `${CONFIG.PING_URL}?_t=${Date.now()}`
				...
				if (response.ok) {
					...
					if (text.includes('"message"') || text.includes('pong') || text.length < 100) {
						success = true
						this._lastPingSuccess = Date.now()
						break
					} else {
				...
				if (error.name === 'AbortError') {
					...
					// Check for "hard" network errors (DNS, No Route, NIC down)
					if (error.message?.includes('Failed to fetch') || 
						error.message?.includes('NetworkError') ||
						error.message?.includes('DNS_PROBE_FINISHED')) {
						isHardError = true
					}
				}
		...
		if (success) {
			...
			// Ping success is the ONLY way to set browserOnline to true
			if (!offlineState._browserOnline) {
				offlineState._browserOnline = true
				offlineState._notifyChange('ping-success')
			}
			...
			// Immediate offline on hard error (like NIC down)
			if (isHardError || this._consecutiveFailures >= CONFIG.OFFLINE_THRESHOLD) {
				if (offlineState._serverOnline) {
					log.warn(`Server offline (failures: ${this._consecutiveFailures}, isHardError: ${isHardError})`)
...
class OfflineStateManager {
	...
	_initBrowserListeners() {
		window.addEventListener('online', () => {
			log.debug('Browser online event (verification required)')
			// DO NOT set browserOnline = true immediately
			// It's often a false positive if a NIC is up but has no route
	...
	setManualOffline(value, { silent = false } = {}) {
		...
		// If turning off manual mode, force immediate verification ping
		if (!newValue) {
			this._networkMonitor.checkNow()
		}
	...
	initialize(initialState = {}) {
		...
		// Use navigator.onLine as a fallback but don't trust it if it's true
		// Many browsers return true even if there is no default route
		const browserMaybeOnline = typeof navigator !== 'undefined' ? navigator.onLine : false

		// Verification will happen in the first ping
		this._serverOnline = true
		this._manualOffline = initialState.manualOffline || false
		this._browserOnline = true
		this._initialized = true
		this._previousIsOffline = false // Start as online
		...
		log.info('Offline state initialized (Strict Mode)', this.getState())
	}
