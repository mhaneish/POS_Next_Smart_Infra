...
export async function call(method, params) {
	try {
		...
		// Network errors during an actual API call are the most reliable signal of being offline
		if (error && (
			error.message?.includes('Failed to fetch') || 
			error.message?.includes('NetworkError') ||
			error.message?.includes('Load failed')
		)) {
			console.warn(`Network error detected in call(${method}): marking server as offline`)
