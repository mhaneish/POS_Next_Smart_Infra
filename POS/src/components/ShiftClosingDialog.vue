...
async function submitClosing() {
	...
	try {
		...
		// When expected amounts are hidden, close immediately without showing reconciliation details
...
const hasShortPayments = computed(() => {
	if (!closingData.value || !closingData.value.payment_reconciliation) return false
	return closingData.value.payment_reconciliation.some(
		(p) => Number.parseFloat(p.difference || 0) < 0,
	)
})

const validationWarningText = computed(() => {
	if (!isAllFilled.value) {
		return __('Please enter all closing amounts')
	}
	if (hideExpectedAmount.value && hasShortPayments.value) {
		return __('Cannot close shift. Please review amounts.')
	}
	return __('Please complete required fields')
})
