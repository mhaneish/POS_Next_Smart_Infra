			...
			>
				<div class="flex min-h-screen flex-col items-center justify-center px-4 py-4 text-center">
					<Transition name="dialog-content">
						<div
							v-if="show"
							class="my-8 inline-block w-full max-w-5xl transform overflow-hidden rounded-xl bg-white text-left align-middle shadow-xl dialog-content z-dialog-content"
						>
							...
							<div class="bg-white px-4 pb-6 pt-5 sm:px-6">
								<div class="flex">
									<div class="w-full flex-1">
										...
										<!-- Body Content -->
										<div v-if="localItem" class="flex flex-col md:flex-row gap-4">
											...
											<!-- POS NUMPAD -->
											<div v-if="showNumpad" class="w-full md:w-80 lg:w-96 flex-shrink-0 rounded-xl border border-gray-200 bg-white p-4">
												<div class="grid grid-cols-2 gap-2 mb-3">
													<button
														type="button"
														class="numpad-btn bg-gray-100"
														@click="focusQuantityInput"
													>
														{{ __('Quantity') }}
													</button>
													<button
														type="button"
														class="numpad-btn bg-gray-100"
														:disabled="!settingsStore.allowItemDiscount"
														@click="focusDiscountInput"
													>
														{{ __('Discount') }}
													</button>
												</div>
												<div class="mb-2 text-sm text-gray-600 text-center">
													...
													<span class="font-semibold text-blue-600">
														{{ activeInput === 'qty' ? __('Quantity') :
															activeInput === 'rate' ? __('Rate') :
															__('Discount') }}
													</span>
												</div>

												<div class="w-full h-14 mb-3 border rounded-lg flex items-center justify-end px-3 text-xl font-semibold bg-gray-50">
													{{ numpadBuffer || '0' }}
												</div>

												<div class="grid grid-cols-3 gap-2">
													<button class="numpad-btn" @click="handleNumpadKey('7')">7</button>
													<button class="numpad-btn" @click="handleNumpadKey('8')">8</button>
													<button class="numpad-btn" @click="handleNumpadKey('9')">9</button>
													<button class="numpad-btn" @click="handleNumpadKey('4')">4</button>
													<button class="numpad-btn" @click="handleNumpadKey('5')">5</button>
													<button class="numpad-btn" @click="handleNumpadKey('6')">6</button>
													<button class="numpad-btn" @click="handleNumpadKey('1')">1</button>
													<button class="numpad-btn" @click="handleNumpadKey('2')">2</button>
													<button class="numpad-btn" @click="handleNumpadKey('3')">3</button>
													<button class="numpad-btn" @click="handleNumpadKey('.')">.</button>
													<button class="numpad-btn" @click="handleNumpadKey('0')">0</button>
													<button class="numpad-btn bg-red-100" @click="handleNumpadKey('backspace')">⌫</button>
												</div>

												<div class="grid grid-cols-2 gap-2 mt-2">
													<button class="numpad-btn bg-gray-100" @click="handleNumpadKey('clear')">
														{{ __('Clear') }}
													</button>
													<button class="numpad-btn bg-blue-200 text-black hover:bg-blue-300" @click="confirmNumpad">
														{{ __('Apply') }}
													</button>
												</div>
											</div>
...
watch(
	...
	(newItem) => {
		if (newItem) {
			...
			numpadBuffer.value = String(localQuantity.value || "")
			nextTick(() => {
...
function activateInput(type) {
  ...
  numpadBuffer.value = getCurrentValueAsString()
}
...
function getCurrentValueAsString() {
  if (activeInput.value === 'qty') return String(localQuantity.value || '')
  if (activeInput.value === 'rate') return String(localRate.value || '')
  if (activeInput.value === 'discount') return String(discountValue.value || '')
  return ''
}

function applyBufferValue() {
  const value = parseFloat(numpadBuffer.value)
  if (isNaN(value)) return

  if (activeInput.value === 'qty') {
    localQuantity.value = value
    calculateTotals()
  }

  if (activeInput.value === 'rate') {
...
function handleNumpadKey(key) {
  ...
  if (key === '.') {
    if (!numpadBuffer.value.includes('.')) {
      numpadBuffer.value += '.'
    }
    return
  }
