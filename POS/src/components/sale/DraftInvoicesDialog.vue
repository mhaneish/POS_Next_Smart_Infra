	...
	>
		<template #body-content>
			<div class="flex flex-col gap-3">
				<div v-if="drafts.length > 0" class="relative">
					<input
						v-model="searchTerm"
						type="text"
						:placeholder="__('Search drafts by ID, customer or item...')"
...
const filteredDrafts = computed(() => {
	...
	return drafts.value.filter((d) => {
		const id = d.draft_id?.toLowerCase() || ""
		const customer =
			(typeof d.customer === "string" ? d.customer : (d.customer?.customer_name || d.customer?.name || ""))?.toLowerCase() || ""
		const items = Array.isArray(d.items) ? d.items.map(i => (i.item_name || i.item_code || "")?.toLowerCase()) : []
		const dateStr = new Date(d.created_at).toLocaleString(DEFAULT_LOCALE).toLowerCase()
		return (
			id.includes(term) ||
