...
const { allowReturnWithoutInvoice } = storeToRefs(posSettingsStore)
const localAllowReturnWithoutInvoice = ref(false)
const showReturnButton = computed(
	() => !isOffline.value && (allowReturnWithoutInvoice.value || localAllowReturnWithoutInvoice.value),
)
...
const loadPaymentMethodsResource = createResource({
	...
	makeParams() {
		return {
			...
			fields: JSON.stringify(["name", "payments", "warehouse", "customer", "company", "currency"]),
		}
	},
	auto: false,
	onSuccess(data) {
		...
		defaultWarehouse.value = data?.warehouse || ""
		defaultCustomer.value = data?.customer || ""
	},
	onError(error) {
		console.error("Error loading payment methods:", error)
		if (paymentMethodsRetry < 2 && props.posProfile) {
			paymentMethodsRetry++
			setTimeout(() => {
				loadPaymentMethodsResource.reload()
			}, 1000 * paymentMethodsRetry)
		}
	},
})
let paymentMethodsRetry = 0

// Resource to fetch POS Settings flag as a fallback after cache clears
...
// Lifecycle hooks
onMounted(() => {
	if (props.posProfile && !posSettingsStore.isLoaded) {
		posSettingsStore.loadSettings(props.posProfile)
	}
	if (props.posProfile) {
		loadPaymentMethodsResource.reload()
		loadAllowReturnFlagResource.submit()
	}
...
watch(
	...
	(newProfile) => {
		if (newProfile) {
			posSettingsStore.loadSettings(newProfile)
			loadPaymentMethodsResource.reload()
			loadAllowReturnFlagResource.submit()
		}
	},
