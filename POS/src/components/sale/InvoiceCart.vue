	...
	<div class="flex flex-col h-full bg-white">
		...
		<div class="px-2.5 py-2 border-b border-gray-200 bg-gray-50">
			...
			<div ref="customerSearchContainer" class="relative">
				...
				<div v-else>
					<div class="flex gap-1.5 items-start">
						...
						<div class="flex-1 flex flex-col gap-2">
							...
							<div v-if="showCustomerNumpad" class="grid grid-cols-4 gap-2">
								<button
									v-for="d in ['1','2','3','4','5','6','7','8','9']"
									:key="d"
									@mousedown.prevent
									@click="appendCustomerDigit(d)"
									class="h-10 rounded-lg border border-gray-200 bg-white hover:bg-blue-50 text-sm font-semibold"
								>
									{{ d }}
								</button>
								<button
									@mousedown.prevent
									@click="backspaceCustomerDigit"
									class="h-10 rounded-lg border border-gray-200 bg-white hover:bg-blue-50 text-sm font-semibold"
								>
									⌫
								</button>
				...
				<!-- Customer Dropdown -->
				<div
					v-if="customerSearchFocused || showCustomerNumpad || customerSearch.trim().length >= 2"
				...
				>
					...
					<div
						v-if="(customerSearchFocused || showCustomerNumpad) && customerSearch.trim().length < 2 && customerResults.length > 0"
		...
		<div class="p-1.5 sm:p-2 bg-white border-t border-gray-200">
			...
			<div class="flex flex-col gap-1.5">
				<!-- Checkout Button (Full width when return is enabled) -->
				<button
					type="button"
					@click="handleProceedToPayment"
					:disabled="items.length === 0"
					:class="[
						'w-full py-2.5 px-3 rounded-lg font-bold text-xs text-white transition-all flex items-center justify-center touch-manipulation',
						items.length === 0
				...
				<!-- Return and Hold Buttons (Side by side when return is enabled) -->
				<div class="flex gap-1.5">
					<!-- Return Without Invoice Button -->
					<button
						v-if="items.length > 0 && showReturnButton"
						type="button"
						@click="handleReturnItems"
						:disabled="isSubmittingReturn"
						class="flex-1 py-2 px-3 rounded-lg font-bold text-xs text-red-700 bg-red-50 border border-red-200 hover:bg-red-100 active:bg-red-200 transition-all flex items-center justify-center touch-manipulation active:scale-[0.98] shadow-sm"
					>
						<svg
						...
						<div v-else class="animate-spin rounded-full h-4 w-4 border-2 border-red-600 border-t-transparent me-1.5"></div>
						<span>{{ isSubmittingReturn ? __("Processing...") : __("Return Items") }}</span>
					</button>
					...
					<button
						...
						:class="[
							'py-2 px-2 rounded-lg font-semibold text-xs transition-all touch-manipulation active:scale-[0.98] flex items-center justify-center',
							showReturnButton ? 'flex-1 bg-amber-50 text-amber-700 hover:bg-amber-100 active:bg-amber-200 border border-amber-100' : 'flex-1 bg-orange-50 text-orange-700 hover:bg-orange-100 active:bg-orange-200'
						]"
		...
		<Dialog
			v-model="showReturnConfirmation"
			:options="{
		...
		>
			<template #body-content>
				<div class="p-4 text-center">
					...
					<h3 class="text-lg font-bold text-gray-900 mb-2">{{ __('Process Return Invoice?') }}</h3>
					<p class="text-sm text-gray-600 mb-6">
						{{ __('You are about to return items with a total value of {0}.', [formatCurrency(props.grandTotal), __(returnPaymentMethodName)]) }}
					</p>
					<div class="flex gap-3">
						<button
							type="button"
							@click="showReturnConfirmation = false"
							class="flex-1 py-2.5 px-4 rounded-lg font-bold text-sm text-gray-700 bg-gray-100 hover:bg-gray-200 transition-colors"
						>
							{{ __('Cancel') }}
						</button>
						<button
							type="button"
							@click="confirmReturn"
							class="flex-1 py-2.5 px-4 rounded-lg font-bold text-sm text-white bg-red-600 hover:bg-red-700 shadow-md transition-colors"
						>
							{{ __('Confirm Return') }}
						</button>
					</div>
...
const { allowReturnWithoutInvoice } = storeToRefs(settingsStore);
const localAllowReturnWithoutInvoice = ref(false);
const showReturnButton = computed(() => !isOffline.value && (allowReturnWithoutInvoice.value || localAllowReturnWithoutInvoice.value));
const isSubmittingReturn = ref(false);
const showReturnConfirmation = ref(false);
const returnPaymentMethodName = ref("");
...
const loadAllowReturnFlagResource = createResource({
	...
	auto: false,
	onSuccess(data) {
		localAllowReturnWithoutInvoice.value = Boolean(data?.allow_return_without_invoice);
	},
	onError(error) {
		console.error("Error loading allow_return_without_invoice flag:", error);
	},
});
...
const createReturnResource = createResource({
	...
	onError(err) {
		isSubmittingReturn.value = false;
		let errorMessage = err.message || __("Failed to create return invoice");
		if (err._server_messages) {
			try {
				const messages = JSON.parse(err._server_messages);
				const firstMsg = JSON.parse(messages[0]);
				if (firstMsg.message) errorMessage = firstMsg.message;
			} catch (e) {
				// ignore
			}
		}
...
const handleReturnItems = async () => {
	...
	const defaultPayment = shiftStore.currentProfile?.payments?.find(p => p.default) || shiftStore.currentProfile?.payments?.[0];
	const payments = [];
	if (defaultPayment) {
		payments.push({
			mode_of_payment: defaultPayment.mode_of_payment,
			account: defaultPayment.default_cash_account || defaultPayment.account,
			amount: -Math.abs(props.grandTotal)
		});
		returnPaymentMethodName.value = defaultPayment.mode_of_payment;
	} else {
		returnPaymentMethodName.value = "None";
	}
	...
	const invoiceData = {
		...
		items: props.items.map(item => ({
			...
			uom: item.uom,
			warehouse: item.warehouse || props.warehouses[0]?.warehouse,
			serial_no: item.serial_no,
			batch_no: item.batch_no,
			conversion_factor: item.conversion_factor || 1
		})),
		payments: payments,
		remarks: __("Return items without invoice")
	};

	returnInvoiceData.value = invoiceData;
	showReturnConfirmation.value = true;
};

const confirmReturn = () => {
	if (!returnInvoiceData.value) return;
	showReturnConfirmation.value = false;
	isSubmittingReturn.value = true;
	createReturnResource.submit({ invoice: JSON.stringify(returnInvoiceData.value), data: JSON.stringify({}) });
};
...
const showCustomerNumpad = ref(false);
...
function selectCustomer(cust) {
	...
	selectedIndex.value = -1;
	customerSearchFocused.value = false;
	showCustomerNumpad.value = false;
...
onMounted(() => {
	...
	// Load allow_return_without_invoice flag as fallback
	if (props.posProfile) {
		loadAllowReturnFlagResource.reload();
	}
});
...
function appendCustomerDigit(digit) {
	const next = `${customerSearch.value || ""}${digit}`;
	customerSearch.value = next;
}

function backspaceCustomerDigit() {
	const current = customerSearch.value || "";
	if (!current) return;
	customerSearch.value = current.slice(0, -1);
}

function clearCustomerDigits() {
	customerSearch.value = "";
}
</script>
```
