	...
	<div class="flex flex-col h-full bg-gray-50">
		...
		<div class="px-1.5 sm:px-3 pt-1.5 sm:pt-3 pb-1.5 sm:pb-2 bg-white border-b border-gray-200">
			<div class="flex items-center gap-1 sm:gap-2 overflow-x-auto pb-1 scrollbar-hide snap-x snap-mandatory">
				<button
					@click="showItemGroupDialog = true"
					:class="[
					...
					:aria-label="__('Open item group selector')"
				>
		...
		<Dialog v-model="showItemGroupDialog" :options="{ title: __('Select Item Group'), size: 'lg' }">
			<template #body-content>
				<div class="py-4 max-h-[70vh] overflow-y-auto">
					<div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
						<button
							@click="handleItemGroupSelection(null)"
							:class="[
								'py-4 px-3 rounded-xl text-xs sm:text-sm font-semibold transition-all touch-manipulation border-2',
								!selectedItemGroup
						...
						<button
							v-for="group in itemGroups"
							:key="group.item_group"
							@click="handleItemGroupSelection(group.item_group)"
							:class="[
								'py-4 px-3 rounded-xl text-xs sm:text-sm font-semibold transition-all touch-manipulation border-2',
								selectedItemGroup === group.item_group
									? 'bg-blue-600 text-white border-blue-600 shadow-lg'
					...
					<div v-if="!itemGroups || itemGroups.length === 0" class="text-center py-8 text-xs text-gray-500">
						{{ __('No item groups available') }}
					</div>
				</div>
			</template>
		</Dialog>
...
const showItemGroupDialog = ref(false)
...
function handleItemGroupSelection(groupName) {
	itemStore.setSelectedItemGroup(groupName || null)
	showItemGroupDialog.value = false
}
