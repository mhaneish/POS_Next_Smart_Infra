			...
			>
				...
				>
					...
					<div v-if="lastSelectedMethod && remainingAmount > 0" class="hidden lg:block" :class="isCompactMode ? 'mb-2' : 'mb-3'">
						...
						<div class="grid gap-1.5" :class="quickAmountsDisplay.length === 1 ? 'grid-cols-1' : 'grid-cols-4'">
							<button
								v-for="amount in quickAmountsDisplay"
								:key="amount"
								...
								:class="[
									...
									isCompactMode ? 'px-4 py-3 text-base' : 'px-4 py-3 text-base',
					...
					<div class="lg:hidden flex flex-col" :class="isSmallMobile ? 'gap-1' : 'gap-1.5'">
						...
						<div v-if="lastSelectedMethod && remainingAmount > 0" :class="['space-y-1 flex-shrink-0', isSmallMobile ? 'mb-1' : 'mb-1.5']">
							...
							<div class="grid" :class="[quickAmountsDisplay.length === 1 ? 'grid-cols-1' : 'grid-cols-4', isSmallMobile ? 'gap-0.5' : 'gap-1']">
								<button
									v-for="amount in quickAmountsDisplay"
									:key="amount"
									@click="addCustomPayment(lastSelectedMethod, amount)"
									...
									:class="[
										...
										isSmallMobile ? 'py-2 text-xs' : 'py-2.5 text-sm',
										isQuickAmountDisabled(amount)
											? 'bg-gray-50 border-gray-100 text-gray-300 cursor-not-allowed'
...
const allowsOverpayment = computed(() => {
	// If exact amount mode is active, NO overpayment allowed (disables change for all methods)
	if (isExactAmountModeActive.value) return false
...
const isExactAmountValid = computed(() => {
	...
	// If exact amount mode is active, total paid must not exceed grand total (no change allowed)
...
const quickAmountsDisplay = computed(() => {
	const amount = roundCurrency(remainingAmount.value)
...
watch(
	() => [show.value, paymentMethods.value.length],
	() => {
		if (show.value) {
			applyDefaultFullPayment()
		}
...
function applyDefaultFullPayment() {
	...
	lastSelectedMethod.value = defaultMethod

	const amount = roundCurrency(props.grandTotal)
	if (amount <= 0) return
	if (
		isWalletPaymentMethod(defaultMethod.mode_of_payment) &&
		availableWalletBalance.value <= 0
	) {
		return
	}

	addCustomPayment(defaultMethod, amount)
}
...
function quickAddPayment(method) {
	...
	// Exact amount validation for non-cash payments or all payments when exact mode is active
	if (isExactAmountModeActive.value) {
		...
		// For quick add (long press), use exact remaining balance to complete payment
		amt = maxAllowed
	}
...
function addCustomPayment(method, amount) {
	...
	if (isExactAmountModeActive.value) {
		...
		if (maxAllowed <= 0) {
			showWarning(__("Invoice fully paid. No additional payment needed."))
			return
		}

		// All methods (Cash & Non-cash): cannot exceed remaining balance (No change allowed)
		if (roundCurrency(amt) > maxAllowed) {
			showWarning(
				__("Payment cannot exceed the remaining balance {0}", [
		...
		// Update the amount to match maxAllowed if it's within threshold (optional UX, but sticking to user's "not exceed" rule)
