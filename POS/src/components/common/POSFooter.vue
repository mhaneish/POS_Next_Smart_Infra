...
const linkText = ref('Smart Infra')
const footerLink = ref('https://smartinfra.ly/')
...
const brandSignature = computed(() => `SmartInfra-${componentId}`)
...
const loadBrandingConfig = async () => {
	try {
		...
		if (response) {
			...
			// Override with desired branding
			const desiredText = 'Powered by'
			const desiredBrand = 'Smart Infra'
			const desiredUrl = 'https://smartinfra.ly/'

			footerText.value = desiredText
			linkText.value = desiredBrand
			footerLink.value = desiredUrl
			serverValidationEnabled.value = false

			// Persist desired values into config for integrity checks
			config.value._t = btoa(desiredText)
			config.value._l = btoa(desiredBrand)
			config.value._u = btoa(desiredUrl)
			config.value._v = false
		...
		linkText.value = 'Smart Infra'
		footerLink.value = 'https://smartinfra.ly/'
	}
}
...
const validateWithServer = async () => {
	...
	try {
		...
		console.error('[Branding] Server validation failed:', error)
	}
}
...
const logClientEvent = async (eventType, details = {}) => {
	try {
		...
		console.error('[Branding] Failed to log event:', error)
	}
}
...
const ensureBranding = () => {
	...
	const expectedBrand = atob(config.value._l || btoa('Smart Infra'))
	const expectedUrl = atob(config.value._u || btoa('https://smartinfra.ly/'))
