					...
					<div class="flex gap-2">
						...
						<Button
							...
							@click="
								() => {
									handlePrintInvoice({
										name: uiStore.lastInvoiceName,
										doc: uiStore.lastInvoiceDoc,
									});
...
async function handlePaymentCompleted(paymentData) {
	try {
		...
		if (offlineStore.isOffline) {
			...
			const offlineInvoiceId = `OFFLINE-${Date.now()}`;
			...
			uiStore.showSuccess(
				...
				{
					...invoiceData,
					name: offlineInvoiceId,
					customer_name: cartStore.currentCustomer?.customer_name || invoiceData.customer,
					company: shiftStore.currentProfile?.company,
					owner: shiftStore.currentProfile?.owner || "Cashier",
				...
				if (shiftStore.autoPrintEnabled) {
					try {
						await handlePrintInvoice({
							name: invoiceName,
							doc: result, // Pass result as doc if available
						});
					...
					uiStore.showSuccess(invoiceName, invoiceTotal, paidAmount, result);
...
async function handlePrintInvoice(invoiceData) {
	try {
		// 1. Determine if we must use offline logic
		const isOfflineInvoice = invoiceData.name && invoiceData.name.startsWith("OFFLINE-");
		const isCurrentlyOffline = offlineStore.isOffline;

		if (isCurrentlyOffline || isOfflineInvoice) {
			if (invoiceData.doc) {
				log.info("Printing offline using available document data");
				await printInvoice(invoiceData.doc);
				return;
			}
		}

		// 2. Online flow: Try the standard online method (fetches latest data + settings)
		// This is preferred as it respects POS Profile's print format and letterhead
		try {
			await printInvoiceByName(invoiceData.name);
		} catch (onlineError) {
			log.warn("Online print failed, attempting fallback with local data", onlineError);
			
			// 3. Fallback: If online failed but we have the doc in memory, use it
			if (invoiceData.doc) {
				await printInvoice(invoiceData.doc);
			} else if (invoiceData.items && Array.isArray(invoiceData.items)) {
				// Also handle if the full invoice object itself was passed instead of {name, doc}
				await printInvoice(invoiceData);
			} else {
				throw onlineError; // Re-throw if no local data available to fall back to
			}
		}
	} catch (error) {
